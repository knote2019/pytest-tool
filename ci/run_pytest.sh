...
if [[ ".py" =~ "${CI_CASES_REGEX}" ]];then
    ...
    echo pytest ${CI_CASES_REGEX}/*.py --capture=no --clean-alluredir --alluredir=${CI_TEST_REPORT_PATH}
    pytest ${CI_CASES_REGEX}/*.py --capture=no --clean-alluredir --alluredir=${CI_TEST_REPORT_PATH}
fi
